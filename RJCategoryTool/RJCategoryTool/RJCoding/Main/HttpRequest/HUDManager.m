...
+(void)showAlertWithCustomImage:(NSString *)imageName title:(NSString *)title inView:(UIView *)view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([imageName isKindOfClass:[UIImage class]]) {//五五来客对这段代码做了一点修改，我们代码里面传的都是UIImage，做了兼容
            littleView.image = (id)imageName;
        }else{
...
+(void)hideAlert{
    dispatch_async(dispatch_get_main_queue(), ^{
        MBProgressHUD *hud = [HUDManager HUDForView:gloomyView];
