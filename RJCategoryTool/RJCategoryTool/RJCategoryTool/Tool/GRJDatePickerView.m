...
@property (nonatomic,assign)GRJDateStyle datePickerStyle;
...
@implementation GRJDatePickerView
...
-(void)setupUI {
    ...
    self.doneButtonColor = rj_RGBAColor(247, 133, 51, 1);
    self.frame=CGRectMake(0, 0, rj_kScreenWidth, rj_kScreenHeight);
    ...
    self.bottomConstraint.constant = -self.grj_height;
    self.backgroundColor = rj_RGBAColor(0, 0, 0, 0);
    [self layoutIfNeeded];
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        ...
        label.textColor =  rj_RGBAColor(247, 133, 51, 1);
...
-(void)show {
    ...
    [UIView animateWithDuration:.3 animations:^{
        if (rj_kIsiPhoneX) {
            self.bottomConstraint.constant = 10 + 34;
        } else {
            self.bottomConstraint.constant = 10;
        }
        self.backgroundColor = rj_RGBAColor(0, 0, 0, 0.4);
